...
def update(frame, *args):
    ...
    for i in range(N):
        for j in range(N):
            # shift gaussian
            shifted_gaussian = np.roll(
                np.roll(gaussian_weights, i - N // 2, axis=0), j - N // 2, axis=1
            )
    ...
    sync = np.isclose(
        new_state, S, atol=θ
    )  # precision really affects emergent behaviour
    ...
    if np.any(mask_ensemble):
        ...
        for i, j in ensemble_idxs:
            ...
            S = np.where(
                shifted_gaussian >= 0, S * (1 - weights) + S[i, j] * weights, S
            )
    ...
    ax1.set_title(
        f"Threshold (ε): {ε}; R: {radius}; σ: {σ}, Update gates: {gate_update}; Precision: {θ}"
    )
    return mat1, mat2
