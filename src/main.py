...
def update(frame, *args):
    ...
    if extended:
        ...
        neighbors = np.stack(
            [left, right, up, down, upper_left, upper_right, bottom_left, bottom_right],
            axis=0,
        )
    ...
    new_state = np.array(
        [[gate[i, j](neighbors[:, i, j]) for j in range(N)] for i in range(N)]
    )
    ...
    ax1.set_title(
        f"Threshold (ε): {ε}; Fixed (k: {k if not fix else None}): {fix}; Φ $\geq$ ε: {geq_cond}; Extended: {extended}"
    )
    return mat1, mat2
